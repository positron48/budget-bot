...
Переименуй ListCommand в ListTablesCommand. Проверь тесты и readme, для проверки всегда запускай make cs-fix && make ci

---

Добавь команду /list [месяц] [год], которая будет спрашивать тип транзакций (расход или доход) и выводить с пагинацией список транзакций из таблицы за выбранный месяц и год (если не указаны то импользовать текущие). Транзакции должны выводиться в обратном порядке - от последних строк к первым
Обнови тесты, readme и весь код, где это нужно.
