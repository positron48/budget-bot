...
1. Сгенерировать protobuf-клиентов из `proto/`:
   - `make proto`
2. Подготовить переменные окружения:
   - Скопируйте `env.example` в `.env` и укажите `TELEGRAM_BOT_TOKEN`.
   - Для локального эмулятора Telegram задайте `TELEGRAM_API_BASE_URL=http://127.0.0.1:3001/`.
   - Убедитесь, что `GRPC_SERVER_ADDRESS=127.0.0.1:8080` (ваш backend).
3. Сборка и запуск (фейки без gRPC):
   - `go build -o bin/budget-bot ./cmd/bot`
   - `./bin/budget-bot`
4. Сборка и запуск с реальными gRPC-клиентами:
   - `go build -tags withgrpc -o bin/budget-bot ./cmd/bot`
   - `./bin/budget-bot`

Примечания:
- Для эмулятора Telegram базовый URL может быть указан как `http://127.0.0.1:3001/` или уже в формате `.../bot%s/%s` — приложение нормализует адрес.
- В режиме long polling эмулятор может логировать частые запросы, даже если запрос фактически удерживается 30 секунд — это особенность эмулятора; код бота корректен.
...
3. **gRPC клиенты**:
   ...
   - [x] Сборка pb из proto (`make proto`)
   - [x] Auth/Transaction/Category/Report/Tenant — заменены на реальные при сборке с тегом `withgrpc`
### gRPC интеграция
...
1. **Система аутентификации**:
   ...
   - [x] gRPC интеграция с Auth (в сборке `withgrpc`)
...
1. **Парсер сообщений**:
   ...
   - [x] Валидация и сообщения об ошибках (фидбек пользователю при неуспешном разборе)
...
2. **Команды управления**:
   - `/categories` - список категорий (backend при `withgrpc`, иначе статично)
   - `/map` - управление сопоставлениями (реализовано: "слово = category_id", `--all`, показ текущего)
   - `/unmap` - удаление сопоставлений (реализовано)
   - `/create_category code название` — создание категории (только в `withgrpc`)
   - `/rename_category id новое_название` — переименование (только в `withgrpc`)
   - `/delete_category id` — удаление (только в `withgrpc`)
...
### gRPC интеграция (реальные клиенты)

- Переключение на реальные клиенты выполняется билд‑тегом `withgrpc`.
- Адрес и режим защиты берутся из конфигурации:
  - `GRPC_SERVER_ADDRESS` (по умолчанию `127.0.0.1:8080`)
  - `GRPC_INSECURE` (true/false; по умолчанию true)
- Поддерживаемые клиенты:
  - Auth: реальный клиент (только в сборке `withgrpc`), по умолчанию — fake
  - Category: реальный, метод `ListCategories`
  - Transaction: реальный, метод `CreateTransaction`
  - Tenant: реальный, метод `ListMyTenants`
  - Report: реальный, метод `GetMonthlySummary` (текущая реализация `/stats`)
- Передача токенов — через gRPC metadata `authorization: Bearer <token>`

Пример запуска с реальными клиентами:
...
TELEGRAM_API_BASE_URL=http://127.0.0.1:3001/ \
go run -tags withgrpc ./cmd/bot
```

### Статус реализованных команд/фич

- `/login`, `/register`, `/logout` — реализованы; сессии хранятся в SQLite
- Парсинг произвольных сообщений — реализован (дата/сумма/валюта/описание)
- Категории — список с backend (при `withgrpc`), фолбэк — статический
- Управление категориями — `/create_category`, `/rename_category`, `/delete_category` (только при `withgrpc`)
- Сохранение транзакций — при `withgrpc` отправка в backend; иначе fake
- `/stats` — использует `ReportService.GetMonthlySummary`
- `/top_categories`, `/recent` — пока заглушки; будет расширено при появлении RPC в backend
- Переключение тенанта — список организаций из backend, смена в сессии планируется
- Метрики Prometheus — доступны при `METRICS_ENABLED=true` на `SERVER_ADDRESS/metrics`

### Примеры команд

- `/stats` — статистика текущего месяца
- `/stats 2025-08` — статистика за август 2025
- `/stats week` — статистика за текущую неделю
- `/top_categories` — топ 5 категорий (текущий месяц)
- `/top_categories 2025-08 10` — топ 10 за август 2025
- `/top_categories week 7` — топ 7 за неделю
- `/recent` — последние 10
- `/recent 20` — последние 20
- `/export` — CSV за текущий месяц
- `/export 2025-08 500` — CSV за август 2025, максимум 500 строк
